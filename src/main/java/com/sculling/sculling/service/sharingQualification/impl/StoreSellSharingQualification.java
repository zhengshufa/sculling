 ...
 * @desc 云连锁店铺卖货分佣逻辑-卖货   //已弃用
 */
@Service
@AllArgsConstructor
public class StoreSellSharingQualification implements SharingQualification {
...
//        String[] sharingProportion = sharingConfig.getStoreSellSharingProportion().split(",");
        ...
        //从本人分起
        Long referralId = sourceMember.getId();
        int count = 0;
        //先赋值全部金额,再减去需要分佣的金额,就是无需分佣的金额,这样就不会因为四舍五入而对不上
        syncOrder.setNonSharingAmount(syncOrder.getAmount());
        //  公司分30%
//        syncOrder.setCompanyProfit(syncOrder.getAmount().multiply(new BigDecimal(sharingConfig.getStoreSellCompanyProportion())));
...
//            if (DateUtil.compare(syncOrder.getCreateTime(), beneficiaryMember.getExpireTime()) < 0) {
            ...
            //已分佣金额
            syncOrder.setSharingAmount((Objects.isNull(syncOrder.getSharingAmount()) ? BigDecimal.ZERO : syncOrder.getSharingAmount()).add(sharingRecord.getAmount()));
            ...
            syncOrder.setSharingGeneration(count);
            count++;
        }
    }
}
