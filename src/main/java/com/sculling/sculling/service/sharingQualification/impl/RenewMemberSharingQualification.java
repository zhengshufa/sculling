...
@AllArgsConstructor
@Slf4j
public class RenewMemberSharingQualification implements SharingQualification {
    ...
    public void sharing(SyncOrder syncOrder) {
        String[] sharingProportion = sharingConfig.getRenewSharingProportion().split(",");
        log.info("会员续费分佣逻辑开始,syncOrder:{}", syncOrder);
        /*
            直推分佣
         */
        ...
        while (count < sharingProportion.length) {
            ...
            //这一层需要分佣的金额
            //按固定值分
            amount = new BigDecimal(sharingProportion[count]);
